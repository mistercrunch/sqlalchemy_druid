"""Integration between SQLAlchemy and Druid.
"""
...
class DruidCompiler(SQLCompiler):
...
class DruidTypeCompiler(compiler.GenericTypeCompiler):
    def visit_CLOB(self, type_, **kw):
        raise ValueError("Druid does not support the CLOB column type.")

    def visit_NCLOB(self, type_, **kw):
        raise ValueError("Druid does not support the NCLOB column type.")

    def visit_DATETIME(self, type_, **kw):
        raise ValueError("Druid does not support the DATETIME column type.")
...
class DruidDialect(default.DefaultDialect):
    ...
    def _get_table_columns(self, connection, table_name, schema):
        ...
        except (druid.DatabaseError, exc.DatabaseError) as e:
            # Normally SQLAlchemy should wrap this exception in sqlalchemy.exc.DatabaseError, which
            # it successfully does in the Hive version. The difference with Druid is that this
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        for row in rows:
            ...
            result.append({
                'name': row.Column,
                'type': coltype,
                # newer Druid no longer includes this column
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        for row in rows:
            ...
            # Druid puts this information in one of 3 places depending on version
